...
export default function FormBPage() {
  ...
  // Definierte Fragen für Variante B (unterschiedlich zu Variante A)
  const variantBQuestions: DialogQuestion[] = [
    {
  ...
  useEffect(() => {
    setQuestions(variantBQuestions)
  }, [])
...
Lassen Sie uns beginnen! Ich stelle Ihnen nacheinander ${questions.length} Fragen. Bei Unklarheiten können Sie gerne nachfragen.
...
${questions[0]?.question}`
      ...
      setChatHistory([{
        role: 'assistant',
        message: 'Entschuldigung, es gab einen Fehler beim Starten des Dialogs. Lassen Sie es uns trotzdem versuchen!',
        timestamp: new Date()
      }])
    } finally {
      setIsLoading(false)
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (currentMessage === '?' || currentMessage.toLowerCase().includes('hilfe') || currentMessage.toLowerCase().includes('help')) {
        const helpResponse = generateHelpResponse(currentQuestion)
        setChatHistory(prev => [...prev, {
          role: 'assistant',
...
${questions.map((q, i) => 
  `• ${q.field}: ${answers[q.id] || currentMessage}`
).join('\n')}
...
Ihre Daten wurden erfasst und können nun gespeichert werden.`
  ...
  const generateHelpResponse = (question: DialogQuestion): string => {
    switch (question.id) {
...
**Beispielantwort:** "Geschätzte Kosten: 100.000 €. Finanzierung über Eigenkapital und KfW-Förderung. Mieterhöhung: ca. 67 €/Monat pro Wohnung (8% p.a. der Kosten)."`
  ...
  const handleSave = async () => {
    try {
      ...
      console.log('Dialog data to save:', dialogData)
            ...
            <CardContent className="space-y-6">
              ...
              <div className="bg-purple-50 p-6 rounded-lg">
                ...
                <ul className="space-y-2 text-purple-700">
                  ...
                  <li className="flex items-start space-x-2">
                    <span className="text-purple-600 mt-1">•</span>
                    <span>Sie beantworten {questions.length} gezielte Fragen nacheinander</span>
                  </li>
                  <li className="flex items-start space-x-2">
                    <span className="text-purple-600 mt-1">•</span>
