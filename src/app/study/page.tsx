...
interface VariantData {
  variant: 'A' | 'B'
  susResults?: { responses: SUSResponses; susScore: number }
...
// SUS Questions (deutsche Übersetzung)
const SUS_QUESTIONS = [
  { id: 'sus_1', text: 'Ich kann mir vorstellen, dieses System regelmäßig zu nutzen.', reverse: false },
  { id: 'sus_2', text: 'Ich finde das System unnötig komplex.', reverse: true },
  { id: 'sus_3', text: 'Ich finde das System einfach zu nutzen.', reverse: false },
  { id: 'sus_4', text: 'Ich denke, ich würde technischen Support benötigen, um das System nutzen zu können.', reverse: true },
  { id: 'sus_5', text: 'Ich finde, dass die verschiedenen Funktionen des Systems gut integriert sind.', reverse: false },
  { id: 'sus_6', text: 'Ich finde, dass das System zu viele Inkonsistenzen aufweist.', reverse: true },
  { id: 'sus_7', text: 'Ich kann mir vorstellen, dass die meisten Menschen das System schnell erlernen.', reverse: false },
  { id: 'sus_8', text: 'Ich finde das System sehr umständlich zu nutzen.', reverse: true },
  { id: 'sus_9', text: 'Ich habe mich bei der Nutzung des Systems sehr sicher gefühlt.', reverse: false },
  { id: 'sus_10', text: 'Ich musste eine Menge lernen, bevor ich mit dem System arbeiten konnte.', reverse: true }
]

// Vertrauen-Fragebogen
const TRUST_QUESTIONS = [
  { key: 'system_reliability', question: 'Ich vertraue darauf, dass das System zuverlässig funktioniert', labels: ['Stimme gar nicht zu', 'Stimme voll zu'] as [string, string] },
  { key: 'data_security', question: 'Ich fühle mich sicher, dass meine Daten geschützt sind', labels: ['Sehr unsicher', 'Sehr sicher'] as [string, string] },
  { key: 'error_handling', question: 'Das System geht angemessen mit Fehlern und Unklarheiten um', labels: ['Sehr schlecht', 'Sehr gut'] as [string, string] },
  { key: 'transparency', question: 'Es ist klar ersichtlich, was das System mit meinen Eingaben macht', labels: ['Gar nicht klar', 'Völlig klar'] as [string, string] },
  { key: 'user_control', question: 'Ich fühle mich im Kontrol über den Eingabeprozess', labels: ['Keine Kontrolle', 'Volle Kontrolle'] as [string, string] }
]
...
function StudyContent() {
  ...
  const calculateSUSScore = (responses: SUSResponses): number => {
    let totalScore = 0
    SUS_QUESTIONS.forEach(question => {
      const response = responses[question.id as keyof SUSResponses]
  ...
  // SUS Questionnaire Component (vereinfacht)
  const SUSQuestionnaire = ({ variant, onComplete }: { variant: 'A' | 'B', onComplete: (data: any) => void }) => {
    const [responses, setResponses] = useState<SUSResponses>({} as SUSResponses)

    const handleResponseChange = (questionId: keyof SUSResponses, value: number) => {
    ...
    const handleSubmit = () => {
      ...
      onComplete({ responses, susScore })
    }

    const completedQuestions = Object.keys(responses).length
    const isComplete = completedQuestions === SUS_QUESTIONS.length

    return (
      <div className="space-y-6">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
          <h3 className="text-xl font-semibold text-blue-900 mb-2">
            📊 System Usability Scale (SUS) - Variante {variant}
          </h3>
          <p className="text-sm text-blue-700 mb-3">
            Bewerten Sie Ihre Erfahrung auf einer Skala von 1 (stimme gar nicht zu) bis 5 (stimme vollständig zu).
          </p>
          <div className="w-full bg-blue-200 rounded-full h-2">
            <div 
              className="bg-blue-600 h-2 rounded-full transition-all duration-300"
              style={{ width: `${(completedQuestions / SUS_QUESTIONS.length) * 100}%` }}
            />
          </div>
          <p className="text-xs text-blue-600 mt-1">
            {completedQuestions} von {SUS_QUESTIONS.length} Fragen beantwortet
          </p>
        </div>
            ...
            <div className="flex justify-center space-x-6">
              {[1, 2, 3, 4, 5].map((num) => (
                <label key={num} className="flex flex-col items-center cursor-pointer group">
                  <input
                    type="radio"
                    name={`sus-${question.id}`}
                    value={num}
                    checked={responses[question.id as keyof SUSResponses] === num}
                    onChange={() => handleResponseChange(question.id as keyof SUSResponses, num)}
            ...
            <CardContent className="p-8">
              <form onSubmit={(e) => {
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Höchster Bildungsabschluss *</label>
                    <select 
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Technische Erfahrung *</label>
                    <select 
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Wie oft füllen Sie Online-Formulare aus? *</label>
                    <select 
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Bevorzugtes Gerät für Formulare</label>
                    <select 
  ...
  if (step === 'variant1_survey') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Bewertung: Variante {variant}</h1>
          ...
          <div className="space-y-8">
              ...
              <CardContent className="p-6">
                <TrustScale
                  trustResults={currentVariantData.trustResults || {
                    system_reliability: 0, data_security: 0, error_handling: 0, transparency: 0, user_control: 0
                  }}
            ...
            <div className="text-center">
              <Button 
                onClick={() => updateStep('variant2_intro')}
  ...
  // Variant 2 Survey - Same structure as variant 1
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="space-y-8">
            ...
            <div className="text-center">
              <Button 
                onClick={() => updateStep('final_survey')}
                disabled={!currentVariantData.susResults || !currentVariantData.trustResults}
                className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3"
              >
            ...
            <CardContent className="p-8">
              <form onSubmit={(e) => {
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    <label className="block text-lg font-semibold mb-4">Abschließende Kommentare</label>
                    <textarea
        ...
        <div className="container mx-auto px-4 max-w-2xl">
          <div className="text-center">
            <div className="mb-6">
              ...
              <h1 className="text-4xl font-bold text-gray-800 mb-2">Vielen Dank!</h1>
              <p className="text-lg text-gray-600">Sie haben die Studie erfolgreich abgeschlossen.</p>
            </div>
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <StudyContent />
        ...
        >
          {isComplete ? 'SUS-Bewertung abschließen' : `Noch ${SUS_QUESTIONS.length - completedQuestions} Fragen`}
        </Button>
      </div>
    )
  }

  // Trust Scale Component (vereinfacht)
            ...
            {[1, 2, 3, 4, 5].map((num) => (
              <label key={num} className="flex flex-col items-center cursor-pointer group">
                <input
                  ...
                  name={`trust-${item.key}`}
                  value={num}
                  checked={trustResults[item.key as keyof TrustQuestionnaire] === num}
